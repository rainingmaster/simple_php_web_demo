...
$ini = new ini();
$hostname = $ini->iniRead("rip--".$Board_IP, "hostname");
$interface = $ini->iniRead("rip--".$Board_IP, "interface");
$interface_ip = $ini->iniRead("rip--".$Board_IP, "ip");

$ping_result = shell_exec("ping -c 1 $Board_IP");
if(strstr($ping_result,"1 received,"))
{
$result = ssh2execu($Board_IP, 'ps -e| grep ripd');
if(strcmp(substr($result, -5,4),"ripd")==0)
{
$no_ripv4_net_flag = 0;
$no_ripng_net_flag = 0;
snmp_set_quick_print(1);
$ripv4_information=snmpget($Board_IP, "public",".1.3.6.1.2.1.23.6.1.0");
$piecesconf=explode(" ",$ripv4_information);
for($i = 0;$i < count($piecesconf);$i ++)
{
      		...
      		for($j = 0,$k=0;strcmp($piecesconf[$i+$j],"network")==0;$j=$j+2,$k++)
		{
			...
			$n = strpos($temp,'!');
                	if ($n) 
                		$ripv4_net_array[$k]=substr($temp,0,$n);
			else
				$ripv4_net_array[$k]=$temp;
              }
      		$ripv4_net_array[$k] = "end";
		$no_ripv4_net_flag = 0;
		break;
        }
	 else
...
<h4>设备<?php echo $Board_IP; ?>的RIP协议配置信息:</h4>
<div id="main">
<table class="features-table">
	...
	<tr>
		...
		<td class="col-cell col-cell1"><width="16" height="16">   
		<?php 
		if($no_ripv4_net_flag == 1)
			echo "路由网段尚未配置";
		else
		{
		for($k = 0;strcmp($ripv4_net_array[$k],"end") != 0;$k++)
			echo $ripv4_net_array[$k].'<br/>';
		}
		?>	
		</td>
	</tr>	
...
<h4>设备<?php echo $Board_IP; ?>的RIP协议参数配置:</h4>
<div id="main">
<table class="features-table">
	<tr>
	...
	<tr>
		<td class="col-cell" >输入要添加的网段（掩码24位）</td>
		<td class="col-cell col-cell1"><width="16" height="16">   
		<form  name="form" method="post"  action="ripv4_add.php?Board_IP=<?php echo $Board_IP; ?>" onSubmit="return checksignup()">
  		<input type="text" name="ripv4_net" id="ripv4_net" class="txt" value="192.168.103.0/24" style="color:#3C3C3C;"  onfocus="if(value=='192.168.103.0/24'){this.style.color='#3C3C3C';value=''}" onblur="if(value==''){this.style.color='#3C3C3C';value='192.168.103.0/24'}" />&nbsp;
	...
	<tr>
		...
		{
		?>
              <form name="form" method="post" action="ripv4_delete.php?Board_IP=<?php echo $Board_IP; ?>" >&nbsp;
	       <select name="ripv4_net_delete">
              <?php
			for($k = 0;strcmp($ripv4_net_array[$k] ,"end") != 0;$k++)
			{
			?>
	       		<option value= <?php echo $ripv4_net_array[$k]; ?> >  <?php echo $ripv4_net_array[$k]; ?> </option>
			<?php
 			}
		?>
	       </select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
{?>
		<script>window.alert('RIP协议进程尚未开启，请先开启进程!');</script>
