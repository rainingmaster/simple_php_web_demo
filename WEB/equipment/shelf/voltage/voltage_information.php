...
<?php
	...
	$device = new Device($ipmb, 1, "Vol", "NULL");
	$getVolSensorLst = $device->getVolSensorLst();	
?>
...
<body>
<?php
	if( count($getVolSensorLst) == 0)
	{
		?>
		<h4>模块<?php echo $name; ?>没有可查询的电压设备</h4>
	<?php
	}
	else
	{
	?>
<h4>模块<?php echo $name; ?>的电压设备信息:</h4>
<div id="main">
<table class="features-table">
	<tr>
		<td class="col-cell col-cell1" >电压设备名称</td>
		<td class="col-cell col-cell1" >IPMB地址</td>
		<td class="col-cell col-cell1" >电压设备ID</td>
		<td class="col-cell col-cell1">当前电压(单位:伏特)</td>
		<td class="col-cell col-cell1">警告电压临界值(单位:伏特)</td>	
	</tr>
	<?php
		for($i = 0; $i < count($getVolSensorLst); $i++) 
		{
	?>
			<tr>
			<td class="col-cell" > <?php echo $getVolSensorLst[$i]->Str_name; ?> </td>
			<td class="col-cell" ><?php echo $getVolSensorLst[$i]->Owner_ipmb; ?></td>
			<td class="col-cell" ><?php echo $getVolSensorLst[$i]->Id; ?></td>
			<td class="col-cell" ><?php echo $getVolSensorLst[$i]->Cur_val; ?></td>
			<td class="col-cell" id="<?php echo $getVolSensorLst[$i]->Id; ?>_box">查询中...
				<script type="text/javascript">
					id_lst.push("<?php echo $getVolSensorLst[$i]->Id; ?>");
				</script>
			</td>
			</tr>
	<?php
...
<div id="main1">
<table class="features-table">
<form name="form" method="post" action="voltage_threshold_setting.php?ipmb=<?php echo $ipmb; ?>" >	
	<tr>
		<td class="col-cell" >警告电压上下限</td>
		...
		上界:<input type="text" name="Upper_critical_threshold" id="Upper_critical_threshold" class="txt" size="6">
              </td>
	</tr>
	<tr>
		<td class="col-cell" >设备ID</td>
		<td class="col-cell col-cell1"><width="16" height="16">
		<select name="voltage_id">
		<?php
		for($i = 0; $i < count($getVolSensorLst); $i++) 
		{
		?>
               	<option value=<?php echo $getVolSensorLst[$i]->Id; ?> ><?php echo $getVolSensorLst[$i]->Id; ?></option>
		<?php
		}
		?>
	       	</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
