...
if($result1 == 1)
{
	...
	{
		$result2 = snmpset($Board_IP,"public",".1.3.6.1.4.1.400.1.1.0","s","3|ip address $ip_address");
		if($result2 == 1)
		{
			$ini->iniWrite("ospf--".$Board_IP, "interface", $interface);
			$ini->iniWrite("ospf--".$Board_IP, "ip", $ip_address);
			$ini->iniUpdate();
		}
		else
		{?>
