...
for($i = 0;$i < count($piecesconf);$i ++)
{
        ...
        {
      		for($j=0,$k=0,$x=0;strcmp($piecesconf[$i+$j],"route")==0;$j=$j+3,$k++)
		{
...
for($i = 0;strcmp($ipv4_net[$i] ,"end") != 0;$i++)
{
	if($i == $ptr)
	{
		$delete_route = $ipv4_net[$i];
		$delete_address = $ipv4_address[$i];
		break;
	}
}
snmpset($Board_IP,"public",".1.3.6.1.2.1.4.24.9.2.0","s","1|no ip route $delete_route $delete_address");
